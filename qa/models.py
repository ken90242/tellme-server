...
class Vote(models.Model):
    VOTE_ACCESSIBLE_MODEL = models.Q(app_label='qa', model = 'answer') | \
                            models.Q(app_label='qa', model = 'question')
    ...
    # GenericForeignKey设置
    content_type = models.ForeignKey(ContentType, related_name='votes_on', on_delete=models.CASCADE, limit_choices_to=VOTE_ACCESSIBLE_MODEL)
...
class Question(models.Model):
    ...
    def status(self):
        ...
        return "C" if self.has_answer else "O"

    @property
    def has_answer(self):
        return any([answer.is_answer for answer in Answer.objects.filter(question=self)])
