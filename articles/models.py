...
from upload.models import ArticleImage

User = get_user_model()

# class ArticleComments(Comment):
...
class Article(models.Model):
    ...
    user = models.ForeignKey(User, null=True, related_name="author", on_delete=models.SET_NULL, verbose_name='作者')
    image = models.ForeignKey(ArticleImage, null=True, related_name="im", on_delete=models.SET_NULL, verbose_name='文章图片')
    ...
    edited = models.BooleanField(default=False, verbose_name='是否已编辑')
    tags = TaggableManager(blank=True, help_text='多个标签使用,(英文)隔开', verbose_name='标签')
    comments = GenericRelation(Comment, object_id_field='object_pk', verbose_name='評論')
